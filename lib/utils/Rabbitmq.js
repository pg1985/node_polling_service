...
class MQManager {
		...
		this.connection = amqp.createConnection({
  		...
  		this.connection.on('ready', () => {
			...
			$connection.queue($config.getAmqpQueue() + '.connect', (q) => {
				...
				q.subscribe($this.startNewConnection);
  			});

			$connection.queue($config.getAmqpQueue() + '.disconnect', (q) => {
	  			q.bind($config.getAmqpExchange(), '#');
				q.subscribe($this.removeConnection);
  			});
	...
	forwardMessageToAPI(message) {
		console.log(this.connection.publish);
		this.connection.publish("post", message, {}, () => {console.log("posted!")});
	}
